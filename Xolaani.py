...
# LOAD FILES OF CURRENT PLAYER CHAMPION
...
# METHOD TO PRINT MESSAGES TO GUI CONSOL
...
# METHOD TO BE CALLED ON IN A THREAD WHEN GAME BEGINS
def script_loop(self, stop_event):
    ...
    while not stop_event.is_set():
        ...
        try:
                ...
                if audio_files == "":
                    ...
                    for player in json.loads(response.text):
                        if player['summonerName'] == IGN:
                            ...
                            # IF STAR GUARDIAN SKINLINE, PLAY STAR GUARDIAN MUSIC
                            if "Star Guardian" in player['skinName']:
                                audio_file_path = os.path.join(script_dir, f"./riot/champ-theme/Star-Guardian.mp3")
                                return_code = subprocess.call(["afplay", audio_file_path])
            ...
            # RESET AUDIO_FILES VAR SO MUSIC PLAYS AT NEXT GAME START
            audio_files = ""
